...
describe('Editor is rendered and can be typed in', () =>
{
    ...
    {
        const {getByTestId} = render(<Editor text={initialText} isTranslation={true} showTranslationBox={dummyFunc} test={true} />);
...
describe('Typing in editor moves the cursor where expected', () =>
{
    ...
    for (let data of testData)
    {
        ...
        {
            //Create the editor
            const {getByTestId} = render(<Editor text="" isTranslation={true} showTranslationBox={() => {}}  test={true} />);
            const textarea = getByTestId('main-textarea');
