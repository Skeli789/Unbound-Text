...
describe('Editor is rendered and can be typed in', () =>
{
    ...
    {
        const {getByTestId} = render(<Editor text={initialText} showTranslate={true} showTranslationBox={dummyFunc} />);
...
describe('Typing in editor moves the cursor where expected', () =>
{
    ...
    for (let data of testData)
    {
        ...
        {
            //Create the editor
            const {getByTestId} = render(<Editor text="" showTranslate={true} showTranslationBox={() => {}} />);
            const textarea = getByTestId('main-textarea');
